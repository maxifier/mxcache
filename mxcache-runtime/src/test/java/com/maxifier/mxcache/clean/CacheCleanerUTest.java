...
public class CacheCleanerUTest {
        ...
        Cleanable<A> ca = new CleanableWithLocks<A>() {
            ...
            public void clearInstance(int id) {
                if (id == 0) {
                    ...
                    t.start();
                    try {
                        // регистраци€ классов не мешает чистить кэш
                        t.join();
        ...
        Cleanable<A> ca = new CleanableWithLocks() {
            ...
            public void clearStatic(int id) {
                if (id == 0) {
                    ...
                    try {
                        t.join(100);
                        // поток, регистрирующий экземпл€р должен выжить
                        assert t.isAlive();
                    } catch (InterruptedException e) {
                        throw new RuntimeException(e);
                    }
    ...
    /**
     * ѕровер€ет, чтобы методы очистки вызывались со всеми необходимыми блокировками
     * @param <T>
     */
    ...
    /**
     * ѕровер€ет, чтобы клинер действительно получал все локи
     */
    ...
    /** ѕровер€ет, чтобы клинер действительно получал нужные локи при очистке по тегу */
    ...
    private @interface MyAnnotation {
        // важно, что класс именно внутренний - тут можно протестирова, чтобы тег бралс€ именно по internal name,
        // а не canonical name.
    }
    ...
    /** ѕровер€ет, чтобы клинер действительно получал нужные локи при очистке по группе */
    ...
    /**
     * «апускает злобный поток, который в течении некоторого времени (30 мс) блокирует некоторые "кэши", не дава€
    ...
    private void startEvilThread(final B b, final Cleanable<A> ca, final Cleanable<B> cb, final Lock lock, final Condition c, final AtomicBoolean bool) {
        Thread t = new Thread("My evil thread") {
            @Override
            public void run() {
                // выполним несколько блокировок-разблокировок.  линер нас должен покорно ждать.
                try {
                    lock.lock();
                    try {
