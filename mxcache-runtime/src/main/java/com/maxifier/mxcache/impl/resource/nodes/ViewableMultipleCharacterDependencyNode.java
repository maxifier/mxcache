 ...
 */
public class ViewableMultipleCharacterDependencyNode extends MultipleDependencyNode implements ResourceViewable {
    private static final Logger logger = LoggerFactory.getLogger(ViewableMultipleCharacterDependencyNode.class);
    ...
    public boolean isChanged() {
        ...
        try {
            for (WeakReference<CleaningNode> ref : instances) {
                ...
                if (node != null) {
                    if (node instanceof ObjectStorage && node instanceof CharacterCache) {
